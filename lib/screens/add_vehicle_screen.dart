...
class _AddVehicleScreenState extends State<AddVehicleScreen> {
  ...
  VehicleType _mapType(String t) {
    if (t == "motorcycle") return VehicleType.motorcycle;
  ...
  void _save() {
    ...
    final typeEnum = _mapType(selectedType);

    dynamic created;

    if (selectedType == "motorcycle") {
      created = Motorcycle.full(
        company,
        autoDate,
        model,
        engine,
    ...
    // ✅ الآن بدل manager/storage: نرسل Event للـ VehicleBloc
    final bloc = context.read<VehicleBloc>();

    if (widget.isEdit) {
      bloc.add(UpdateVehicleEvent(created, typeEnum));
    } else {
      if (insertAtPosition) {
        bloc.add(InsertVehicleAtEvent(idx, created, typeEnum));
      } else {
        bloc.add(AddVehicleEvent(created, typeEnum));
      }
    }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            Row(
              children: [
                Expanded(child: ElevatedButton(onPressed: _save, child: Text(widget.isEdit ? "Save Changes" : "Save"))),
                const SizedBox(width: 12),
